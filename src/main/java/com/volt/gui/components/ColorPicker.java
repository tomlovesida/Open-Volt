...
public class ColorPicker {
    ...
    private void renderColorPreview(DrawContext context, int x, int y) {
        ...
        context.drawBorder(x, y, PREVIEW_SIZE, PREVIEW_SIZE, new Color(100, 100, 100).getRGB());
    }
    ...
    private void renderCheckerboard(DrawContext context, int x, int y, int width, int height) {
        for (int i = 0; i < width; i += 8) {
            for (int j = 0; j < height; j += 8) {
                boolean light = ((i / 8) + (j / 8)) % 2 == 0;
                Color color = light ? new Color(200, 200, 200) : new Color(150, 150, 150);
    ...
    private void drawCrosshair(DrawContext context, int x, int y) {
        context.fill(x - 5, y - 1, x + 5, y + 1, Color.WHITE.getRGB());
        context.fill(x - 1, y - 5, x + 1, y + 5, Color.WHITE.getRGB());
        context.fill(x - 6, y - 2, x + 6, y + 2, Color.BLACK.getRGB());
        context.fill(x - 2, y - 6, x + 2, y + 6, Color.BLACK.getRGB());
    }
    
    private void drawBarIndicator(DrawContext context, int x, int y, int width) {
        context.fill(x - 2, y - 1, x + width + 2, y + 1, Color.WHITE.getRGB());
        context.fill(x - 3, y - 2, x + width + 3, y + 2, Color.BLACK.getRGB());
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isInBounds(mouseX, mouseY, hexX, hexY, hexW, hexH)) { setFocus(true, false, false, false, false); return true; }
        if (isInBounds(mouseX, mouseY, rSliderX, rSliderY, rSliderW, rSliderH)) { draggingR = true; updateRFromMouse(mouseX); return true; }
        if (isInBounds(mouseX, mouseY, rBoxX, rBoxY, rBoxW, rBoxH)) { setFocus(false, true, false, false, false); return true; }
        if (isInBounds(mouseX, mouseY, gSliderX, gSliderY, gSliderW, gSliderH)) { draggingG = true; updateGFromMouse(mouseX); return true; }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        if (isDraggingSquare) { updateSquareColor(mouseX, mouseY); return true; }
        if (isDraggingBrightness) { updateBrightness(mouseY); return true; }
        if (isDraggingAlpha) { updateAlpha(mouseY); return true; }
        if (draggingR) { updateRFromMouse(mouseX); return true; }
        if (draggingG) { updateGFromMouse(mouseX); return true; }
        if (draggingB) { updateBFromMouse(mouseX); return true; }
        if (draggingA) { updateAFromMouse(mouseX); return true; }
    ...
    private void renderControls(DrawContext context, int x, int y, int mouseX, int mouseY) {
        ...
        hexX = x + 28; hexY = rowY; hexW = totalW - 28; hexH = CONTROL_HEIGHT;
        ...
        int boxW = 50; int boxH = CONTROL_HEIGHT;
        int sliderX = x + 28; int sliderW = totalW - 28 - boxW - 6; int sliderH = 8;

        
        context.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, "R", x, rowY + 3, textColor);
        rSliderX = sliderX; rSliderY = rowY + (CONTROL_HEIGHT - sliderH) / 2; rSliderW = sliderW; rSliderH = sliderH;
        UIRenderer.renderSlider(context, rSliderX, rSliderY, rSliderW, rSliderH, colorSetting.getRed() / 255.0, new Color(60,60,60), new Color(255,80,80));
        rBoxX = sliderX + sliderW + 6; rBoxY = rowY; rBoxW = boxW; rBoxH = boxH;
        ...
        gSliderX = sliderX; gSliderY = rowY + (CONTROL_HEIGHT - sliderH) / 2; gSliderW = sliderW; gSliderH = sliderH;
        UIRenderer.renderSlider(context, gSliderX, gSliderY, gSliderW, gSliderH, colorSetting.getGreen() / 255.0, new Color(60,60,60), new Color(80,255,80));
        gBoxX = sliderX + sliderW + 6; gBoxY = rowY; gBoxW = boxW; gBoxH = boxH;
        ...
        bSliderX = sliderX; bSliderY = rowY + (CONTROL_HEIGHT - sliderH) / 2; bSliderW = sliderW; bSliderH = sliderH;
        UIRenderer.renderSlider(context, bSliderX, bSliderY, bSliderW, bSliderH, colorSetting.getBlue() / 255.0, new Color(60,60,60), new Color(80,80,255));
        bBoxX = sliderX + sliderW + 6; bBoxY = rowY; bBoxW = boxW; bBoxH = boxH;
        ...
        if (colorSetting.isHasAlpha()) {
            ...
            aBoxX = sliderX + sliderW + 6; aBoxY = rowY; aBoxW = boxW; aBoxH = boxH;
    ...
    private void drawTextField(DrawContext ctx, int x, int y, int w, int h, String text, boolean focused) {
        Color bg = focused ? new Color(45,45,65,240) : new Color(40,40,55,220);
        ctx.fill(x, y, x + w, y + h, bg.getRGB());
        ctx.drawBorder(x, y, w, h, new Color(100,100,100).getRGB());
        int color = new Color(220,220,220).getRGB();
    ...
    public boolean charTyped(char chr) {
        ...
        boolean any = false;
        if (focusR && Character.isDigit(chr)) { inputR += chr; inputR = clampNumeric(inputR); any = true; }
        if (focusG && Character.isDigit(chr)) { inputG += chr; inputG = clampNumeric(inputG); any = true; }
        if (focusB && Character.isDigit(chr)) { inputB += chr; inputB = clampNumeric(inputB); any = true; }
        if (focusA && Character.isDigit(chr)) { inputA += chr; inputA = clampNumeric(inputA); any = true; }
        if (any) {
            applyInputsIfValid();
            return true;
        }
    ...
    private void setFocus(boolean hex, boolean r, boolean g, boolean b, boolean a) {
        this.focusHex = hex; this.focusR = r; this.focusG = g; this.focusB = b; this.focusA = a;
    }
